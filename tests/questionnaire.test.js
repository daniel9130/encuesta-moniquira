...
test('Todas las preguntas son opcionales incluso Otro y listas borradas',()=>{assert.ok(questions.every(q=>!q.required));assert.deepEqual(validate({}),{});assert.deepEqual(validate({p6:'Otro',edad:''}),{});assert.deepEqual(cleanAnswers({p1:'',p2:'  ',p3:['','','','']}),{});});
