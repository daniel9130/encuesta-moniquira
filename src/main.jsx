...
function Question({q,value,onChange,error}){const id=q.id;return <fieldset className="question" aria-describedby={error?id+'-error':undefined}><legend>{q.label}{q.required?' *':''}</legend>{q.hint&&<p className="hint">{q.hint}</p>}
 ...
 {q.type==='text'&&<><textarea aria-label={q.label} maxLength={1000} rows={3} value={value||''} onChange={e=>onChange(e.target.value)} placeholder="Registre la respuesta"/>{q.allowNS!==false&&q.id!=='observacion'&&<button type="button" className="textbutton" onClick={()=>onChange('No sabe / No responde')}>No sabe / No responde</button>}</>}
 ...
 {value!==undefined&&value!==''&&<button type="button" className="textbutton" onClick={()=>onChange(undefined)}>Dejar sin respuesta</button>}{error&&<small id={id+'-error'} className="error" role="alert">{error}</small>}</fieldset>}
